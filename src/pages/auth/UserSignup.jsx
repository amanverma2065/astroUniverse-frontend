...
function UserSignup() {
    ...
    const handleOnSubmit = async (e) => {
        ...
        if (password !== confirmPassword) {
            ...
            try {
                const sendOtpRes = await axios.post("/api/user/send-otp", { email });
                toast.dismiss(loadingToastId);

                if (sendOtpRes.data.success) {
                    toast.success("OTP sent to email");
                    localStorage.setItem("signupData", JSON.stringify({ firstName, lastName, email, password, confirmPassword, role: "user" }));
                    navigate("/verify-otp");
                }
            } catch (error) {
                toast.dismiss(loadingToastId);
                        ...
                        <div className="p-4 sm:p-6 md:p-8">
                            ...
                            <form onSubmit={handleOnSubmit} className="space-y-4 sm:space-y-6">
                                ...
                                <div className="text-center gap-x-2 flex flex-col sm:flex-row justify-center items-center text-sm sm:text-base">
                                    ...
                                    <NavLink to="/user-login" className="inline-block text-purple-400 font-medium hover:text-purple-300 transition-colors duration-200">
                                        Sign In
                                    </NavLink>
                                </div>
                            </form>
